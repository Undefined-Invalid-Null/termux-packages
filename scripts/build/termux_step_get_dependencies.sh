	...
	while read -r PKG PKG_DIR; do
		...
		if [ "$TERMUX_INSTALL_DEPS" = "true" ] || [ "$cyclic_dependence" = "true" ]; then
			...
			if [ "$force_build_dependency" = "true" ]; then
				...
				# 保存原始值
				local original_repo_app_package_name="$TERMUX_REPO_APP__PACKAGE_NAME"
				local original_app_package="$TERMUX_APP_PACKAGE"
				
				# 允许从官方仓库下载依赖（包名不同时）
				if [ "$TERMUX_APP_PACKAGE" = "com.UIN.Tool" ] && [ "$TERMUX_REPO_APP__PACKAGE_NAME" = "com.termux" ]; then
					echo "INFO: Allowing cross-repo dependency download for $PKG"
					# 临时设置匹配的值以绕过检查
					export TERMUX_REPO_APP__PACKAGE_NAME="$TERMUX_APP_PACKAGE"
				fi
				...
				if ! TERMUX_WITHOUT_DEPVERSION_BINDING="$([ "${cyclic_dependence}" = "true" ] && echo "true" || echo "${TERMUX_WITHOUT_DEPVERSION_BINDING}")" termux_download_deb_pac $PKG $DEP_ARCH $DEP_VERSION $DEP_VERSION_PAC; then
					# 恢复原始值
					export TERMUX_REPO_APP__PACKAGE_NAME="$original_repo_app_package_name"
					export TERMUX_APP_PACKAGE="$original_app_package"
